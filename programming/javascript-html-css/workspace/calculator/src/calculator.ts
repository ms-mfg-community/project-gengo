 ...
 * Multi-language Calculator Implementation
 * 
 * English: Advanced calculator with basic and advanced mathematical operations
 * German: Erweiterte Rechner mit grundlegenden und erweiterten mathematischen Operationen
 * Hindi: बुनियादी और उन्नत गणितीय संचालन के साथ उन्नत कैलकुलेटर
 * Japanese: 基本的および高度な数学演算を備えた高度な電卓
 */
...
// HTML onclick इवेंट्स के लिए ग्लोबल फ़ंक्शन
// HTML onclickイベント用のグローバル関数
...
(window as any).appendToDisplay = (value: string) => {
...
(window as any).clearEntry = () => {
    if (calculatorInstance) {
        calculatorInstance.clearEntry();
    }
};

(window as any).deleteLast = () => {
    if (calculatorInstance) {
