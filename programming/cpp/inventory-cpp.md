...
1. Create a folder named "inventory-management-system" with a subfolder "src"
2. Set up VS Code to use g++ compiler with C++17 support
3. Create a .vscode folder inside the src directory for VS Code configuration
4. Create tasks.json for building the project
```
...
2. Try adding new items with different properties
3. Test removing items by name
4. Search for specific items by name
...
1. Persistent storage (saving/loading inventory from files)
2. More advanced search capabilities (by ID, category, etc.)
